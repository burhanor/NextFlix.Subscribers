...
{
	public class ChannelReceived(IRedisService redisService, IRabbitMqService rabbitMqService, MovieService movieService) : GenericRedisReceived<ChannelModel>(redisService, RedisPrefix.Channel)
	{
		public override async Task<bool> Upsert(string message)
		{
			ChannelModel? model = JsonSerializer.Deserialize<ChannelModel>(message);
			if (model is null)
				return false;

			List<int> movieIds = await movieService.GetMovieIdFromChannelId(model.Id);
			if (movieIds?.Count > 0)
			{
